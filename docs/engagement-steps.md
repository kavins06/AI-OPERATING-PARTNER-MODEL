...
This is the V2 operating sequence for a pre-build AI operating partner engagement. The engagement produces structured organizational intelligence, use-case selection, adoption design, and an implementation blueprint. It does not build agents, MCPs, connectors, live integrations, normalization pipelines, email ingestion, or production automations.

## Step 0: Engagement Tiering And Vertical Extension

Set the engagement tier, scope depth, timeline, offline proof-point boundary, and vertical/regulatory extension needs before discovery begins.
...
Turn the terrain into discovery zones and an explicit candidate use-case shortlist. Each candidate has a hypothesis, target user, workflow boundary, value hypothesis, trust dependencies, adoption dependencies, regulated-domain dependencies, confidence, and disqualification criteria.

## Step 3: Role-Aware AI-Led Interviews

Capture work episodes, decisions, edge cases, source access intelligence, truth production, approvals, adoption realities, and silent evidence needs. Use explicit consent, recording/transcription, retention, PII, no-live-upload, no-asserted-facts, and operating-partner intervention protocols.

## Step 4: Variation Mapping

Use this step when the organization is fragmented, franchised, multi-region, independent-contractor driven, or otherwise too varied for one canonical workflow. Enumerate variants, participation skew, best-practice candidates, and variants requiring separate modeling.
...
Produce structured organizational workflow intelligence with roles, actors, hierarchy, steps, decisions, information objects, source access profiles, truth production profiles, variation patterns, adoption signals, regulated-domain signals, evidence, confidence, unknowns, and completeness checks.
...
Convert prioritized requirements into a canonical machine-readable guidance spec, readable view, behavioral evals, output-quality evals, regulated-domain evals where applicable, truth-production handling, explicit rules, examples, output contracts, owners, and update cadence.
...
Decide build/buy/leverage-vendor, agent topology, model-selection class, UX surface, retrieval/context shape, eval/observability shape, end-user research, incentive alignment, opt-in versus mandate reality, training, rollout, and adoption-failure triggers.
...
Produce an implementation-grade readiness object for each surviving opportunity, including hard gates, behavior-level readiness, dependencies, blockers, evidence confidence, minimum safe first behavior, prohibited behaviors, and Step 17 path.

## Step 17: Implementation Decision Packet

Produce the machine-readable recommendation packet: build-ready brief, gap-remediation plan, governance/data readiness plan, knowledge capture plan, technical feasibility plan, risk/control plan, or do-not-automate recommendation. Include confidence statement, conditional-revision triggers, and sponsor/technical/risk views.

## Step 18: Managed Lifecycle And Adoption-Change Object

Create a machine-readable operating contract for the future capability: ownership, launch gates, validation, monitoring, truth governance, source drift checks, feedback, incidents, revocation, guidance updates, access review, adoption/change management, improvement, expansion, retirement, remediation lifecycle, or no-automation review cadence.
